...
export default function StorefrontShell({
  ...
  const isGym = store.template === 'gym';
  const isHotel2 = store.template === 'hotel2';
  ...
  return (
    <>
      {headerSection && store.template !== 'gym' && store.template !== 'hotel2' ? (
        <StoreHeader headerConfig={headerSection} slug={slug} storeName={store.name} session={session} categories={store.categories} />
      ) : (
        store.template !== 'fitness' && store.template !== 'ironpeak' && store.template !== 'itsolution' && store.template !== 'gym' && store.template !== 'hotel2' && (store.template !== '2m' || !isHomePage) && (
          <Navbar
      ...
      <main className={`flex-grow flex flex-col store-container ${headerSection && store.template !== 'gym' && store.template !== 'hotel2' ? 'pt-16 md:pt-20' : ''} ${store.template === 'ironpeak' || store.template === 'itsolution' || store.template === 'gym' || store.template === 'hotel2' ? '!p-0 !m-0' : ''}`}>
        {children}
      </main>
      <div data-page="footer">
        {store.type !== 'WEBSITE' && store.template !== 'fitness' && store.template !== 'ironpeak' && store.template !== 'dddyou' && store.template !== '1m' && store.template !== 'hotel2' && (store.template !== '2m' || !isHomePage) && <Footer />}
      </div>
      {store.template !== 'fitness' && store.template !== 'ironpeak' && store.template !== 'dddyou' && store.template !== '1m' && store.template !== '2m' && store.template !== 'gym' && store.template !== 'hotel2' && <WhatsAppButton />}
    </>
  );
}
