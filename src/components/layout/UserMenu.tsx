...
export default function UserMenu({ slug, lang, textColor, session }: Props) {
          ...
          {isLoggedIn ? (
            <>
              <div className="px-4 py-3 border-b border-slate-100 mb-1">
                <p className="text-sm font-medium text-slate-900 truncate">{session?.name || user?.displayName || 'User'}</p>
                <p className="text-xs text-slate-500 truncate">{session?.email || user?.email}</p>
              </div>
              ...
              <button
                onClick={async () => {
                  const { logoutCustomer } = await import("@/app/store/actions");
