...
export default function SaleSection({ section, products = [], slug, template }: SaleSectionProps) {
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 md:grid-rows-2 gap-4 h-auto md:h-[800px]">
             {saleProducts.slice(0, 5).map((product, idx) => (
                <Link 
                  ...
                  className={`group relative overflow-hidden rounded-3xl bg-white/5 backdrop-blur-sm shadow-xl ${
          ...
          <div className="space-y-6">
             {saleProducts.slice(0, 4).map((product) => (
                <Link 
                  key={product.id} 
                  href={`/store/${slug}/product/${product.id}`}
                  className="group flex flex-col md:flex-row bg-white/5 backdrop-blur-sm rounded-3xl overflow-hidden shadow-xl hover:shadow-2xl transition-all border border-slate-100/10"
                >
          ...
          <div className="divide-y divide-slate-100/10">
             {saleProducts.map((product) => (
                <Link 
                  key={product.id} 
                  href={`/store/${slug}/product/${product.id}`}
                  className="flex items-center justify-between py-8 group bg-white/5 backdrop-blur-sm px-6 rounded-2xl mb-4"
                >
                   <div className="flex items-center gap-8">
                      <div className="w-20 h-20 rounded-2xl overflow-hidden bg-slate-100/10">
        ...
        return (
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-8">
             {saleProducts.map((product) => (
                ...
                >
                   <div className="w-full aspect-square rounded-full overflow-hidden border-4 border-white/10 shadow-2xl mb-6 relative ring-4 ring-red-500/10 group-hover:ring-red-500/50 transition-all duration-700 bg-white/5 backdrop-blur-sm">
                      <SmartImage src={product.images[0]} className="w-full h-full object-cover group-hover:scale-110 transition-transform" alt={product.name} />
                      <div className="absolute inset-0 bg-red-600/20 mix-blend-overlay opacity-0 group-hover:opacity-100 transition-opacity" />
                   </div>
                   <h3 className="text-sm font-black uppercase tracking-widest text-white group-hover:text-red-600 transition-colors">{product.name}</h3>
  ...
  return (
    <section 
      key={section.id} 
      className="py-32 px-8 transition-all duration-1000 bg-transparent"
    >
