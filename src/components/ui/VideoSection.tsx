...
export default function VideoSection({ section, slug }: VideoSectionProps) {
  ...
  // Support both formats: new builder (config.items) and legacy (config.videos / config.videoUrl)
  const items = Array.isArray(config.items) ? config.items
    : Array.isArray(config.videos) ? config.videos
    : config.videoUrl ? [{ url: config.videoUrl, title: config.title || '', poster: config.posterImage || '' }]
    : [];

  const columns = config.columns || (items.length === 1 ? 1 : items.length === 2 ? 2 : 3);
  const displayStyle = config.displayStyle || 'grid';

  return (
    <section className={`relative py-20 overflow-hidden px-8 max-w-[1800px] mx-auto`}>
      ...
      {config.title && (
        <div className="relative z-10 mb-12 text-center px-8">
          <h2 className="text-4xl md:text-6xl font-black tracking-tighter uppercase mb-4">{config.title}</h2>
          {config.subtitle && <p className="text-slate-500 text-lg max-w-2xl mx-auto">{config.subtitle}</p>}
