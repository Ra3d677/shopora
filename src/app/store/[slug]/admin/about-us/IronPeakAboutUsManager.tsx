...
function buildAbout(settings: any) {
  ...
  return {
    ...
    sectionTitle: a.sectionTitle || "About Us",
    sectionSubtitle: a.sectionSubtitle || "We're More Than Just A Gym",
    heading: a.heading || "Your Fitness Journey Starts Here",
    paragraph1: a.paragraph1 || "",
    paragraph2: a.paragraph2 || "",
    image: a.image || "https://res.cloudinary.com/dno6yitvw/image/upload/v1780997921/shopora/ironpeak/about.jpg",
...
export default function IronPeakAboutUsManager({ slug, initialSettings }: { slug: string; initialSettings: any }) {
  ...
  async function handleSave() {
    setSaving(true);
    try {
      const { enabled, sectionTitle, sectionSubtitle, heading, paragraph1, paragraph2, image, features } = content;
      const res = await saveStoreSettings(slug, {
        ironpeakSettings: {
          ...initialSettings,
          about: { enabled, sectionTitle, sectionSubtitle, heading, paragraph1, paragraph2, image, features },
        },
      });
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">Customize the About section of your IronPeak template</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-3 bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
          ...
          <div className="p-6" style={{ fontFamily: "'Nunito', sans-serif" }}>
            <style>{`
              .ip-preview-about { background: linear-gradient(135deg,#f8f9fa,#e9ecef); padding: 4rem 0; border-radius: 16px; }
              ...
              .ip-preview-about-content { display: grid; grid-template-columns: 1fr 1fr; gap: 4rem; align-items: center; }
              .ip-preview-about-text h3 { font-size: 2rem; margin: 0 0 1.5rem; color: #222; font-weight: 700; }
              .ip-preview-about-text p { font-size: 1.1rem; color: #555; margin: 0 0 2rem; line-height: 1.8; }
              .ip-preview-features-grid { display: grid; grid-template-columns: repeat(2,1fr); gap: 1.5rem; margin-top: 2rem; }
              .ip-preview-feature-item { display: flex; align-items: center; gap: 1rem; padding: 1rem; background: #fff; border-radius: 10px; box-shadow: 0 5px 15px rgba(0,0,0,.05); }
              .ip-preview-feature-icon { width: 50px; height: 50px; background: linear-gradient(135deg,#ff6b35,#f7931e); border-radius: 10px; display: flex; align-items: center; justify-content: center; font-size: 1.5rem; color: #fff; flex-shrink: 0; }
              .ip-preview-about-image { border-radius: 20px; overflow: hidden; box-shadow: 0 20px 60px rgba(0,0,0,.2); }
              .ip-preview-about-image img { width: 100%; height: auto; display: block; }
              @media (max-width: 768px) {
                .ip-preview-about-content { grid-template-columns: 1fr; gap: 2rem; }
                .ip-preview-features-grid { grid-template-columns: 1fr; }
              }
            `}</style>
            <div className="ip-preview-about">
              <div className="ip-preview-container">
                <div className="ip-preview-section-header">
                  <h2>{content.sectionTitle}</h2>
                  <p>{content.sectionSubtitle}</p>
                </div>
                <div className="ip-preview-about-content">
                  <div className="ip-preview-about-text">
                    <h3>{content.heading}</h3>
                    <p>{content.paragraph1}</p>
                    <p>{content.paragraph2}</p>
                    <div className="ip-preview-features-grid">
                      {content.features.map((f: any, i: number) => (
                        <div key={i} className="ip-preview-feature-item">
                          <div className="ip-preview-feature-icon">{f.icon}</div>
                          <div style={{ fontWeight: 600, color: '#333', fontSize: '0.95rem' }}>{f.text}</div>
                        </div>
                      ))}
                    </div>
                  </div>
                  <div className="ip-preview-about-image">
                    <img src={content.image} alt="About" style={{ width: '100%', display: 'block' }} />
                  </div>
                </div>
              </div>
            </div>
