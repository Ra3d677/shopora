...
import { 
  Save, Loader2, AlignLeft, AlignCenter, AlignRight, Type, 
  Image as ImageIcon, Paintbrush, Sliders, Rocket, Undo2, 
  Gift, Shield, Truck, Headphones, Clock, Tag, Percent, Sparkles 
} from "lucide-react";
...
export default function TwoMAboutUsManager({ slug, initialContent }: { slug: string; initialContent: any }) {
  const [content, setContent] = useState(() => {
    ...
    return {
      ...
      paddingTop: base.paddingTop || 50,
      paddingBottom: base.paddingBottom || 60,
      showBtn: base.showBtn || false,
      ...
      featuresSection: base.featuresSection || {
        showSection: true,
        items: [
  ...
  function updateFeaturesSection(field: string, val: any) {
    setContent((prev: any) => ({
      ...prev,
      featuresSection: {
  ...
  function updateFeatureItem(index: number, field: string, val: any) {
    setContent((prev: any) => {
      const items = [...prev.featuresSection.items];
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-slate-200 p-8 space-y-6">
        ...
        {activeTab === 'button' && (
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between bg-slate-50 p-4 rounded-2xl border border-slate-100">
              <div>
                <h3 className="text-xs font-bold text-slate-700">تفعيل زر اتخاذ إجراء (Call to Action Button)</h3>
                <p className="text-[10px] text-slate-400 mt-1">تحديد ما إذا كان سيظهر زر في أسفل نص التفاصيل.</p>
              </div>
              <label className="relative inline-flex items-center cursor-pointer">
                <input 
                  type="checkbox" 
                  className="sr-only peer"
                  checked={content.showBtn === true}
                  onChange={(e) => update('showBtn', e.target.checked)}
                />
                <div className="w-11 h-6 bg-slate-200 rounded-full peer peer-checked:bg-blue-600 transition-all after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:after:translate-x-5"></div>
              </label>
            </div>
            ...
            {content.showBtn && (
              <div className="space-y-6 p-4 border border-slate-100 rounded-2xl bg-slate-50/50">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <input 
                      ...
                      className="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 text-xs font-mono"
                      placeholder="#" 
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-3">
                    <label className="text-xs font-bold text-slate-500 uppercase block">ألوان الزر</label>
                    <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-1">
                        <span className="text-[10px] text-slate-400 block">خلفية الزر</span>
                        <div className="flex gap-1.5">
                          <input 
                            type="color" 
                            value={content.btnBgColor || '#fed700'}
                            onChange={e => update('btnBgColor', e.target.value)}
                            className="w-8 h-8 rounded-lg cursor-pointer border border-slate-200" 
                          />
                          <input 
                            type="text" 
                      ...
                      <div className="space-y-1">
                        <span className="text-[10px] text-slate-400 block">نص الزر</span>
                        <div className="flex gap-1.5">
                          <input 
                            type="color" 
                            value={content.btnTextColor || '#333333'}
                            onChange={e => update('btnTextColor', e.target.value)}
                            className="w-8 h-8 rounded-lg cursor-pointer border border-slate-200" 
                          />
                          <input 
                            type="text" 
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-2">
                      <div className="flex justify-between items-center">
                        <label className="text-[10px] font-bold text-slate-500">حجم الخط</label>
                        <span className="text-[10px] font-bold font-mono text-blue-600">{content.btnFontSize || 12}px</span>
                      </div>
                      <input 
                        type="range"
                        min={10}
                        max={24}
                        value={content.btnFontSize || 12}
                        onChange={e => update('btnFontSize', Number(e.target.value))}
                        className="w-full h-1 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-blue-600"
                      />
                    </div>
                    <div className="space-y-2">
                      <div className="flex justify-between items-center">
                        <label className="text-[10px] font-bold text-slate-500">حواف دائرية</label>
                        <span className="text-[10px] font-bold font-mono text-blue-600">{content.btnBorderRadius || 4}px</span>
                      </div>
                      <input 
                        type="range"
                        min={0}
                        max={30}
                        value={content.btnBorderRadius || 4}
                        onChange={e => update('btnBorderRadius', Number(e.target.value))}
                        className="w-full h-1 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-blue-600"
                      />
                    </div>
                  </div>
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between bg-slate-50 p-4 rounded-2xl border border-slate-100">
              <div>
                ...
                <p className="text-[10px] text-slate-400 mt-1">تحديد ما إذا كان سيظهر شريط الميزات (مثل الشحن المجاني، الدعم الفني، إلخ) في أسفل الصفحة الرئيسية.</p>
              </div>
              <label className="relative inline-flex items-center cursor-pointer">
                <input 
                  type="checkbox" 
                  className="sr-only peer"
                  checked={content.featuresSection?.showSection !== false}
                  onChange={(e) => updateFeaturesSection('showSection', e.target.checked)}
                />
                <div className="w-11 h-6 bg-slate-200 rounded-full peer peer-checked:bg-blue-600 transition-all after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:after:translate-x-5"></div>
              </label>
            </div>

            {content.featuresSection?.showSection !== false && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6" dir="rtl">
                {content.featuresSection?.items?.map((item: any, idx: number) => {
                  return (
