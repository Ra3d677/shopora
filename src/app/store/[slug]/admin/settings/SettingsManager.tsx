              ...
              <div className="space-y-6 animate-in fade-in zoom-in-95 duration-500">
                <div className="admin-card backdrop-blur-3xl rounded-2xl p-6 border admin-border shadow-2xl relative overflow-hidden group">
                  ...
                  <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                   ...
                   <div className="mt-8 bg-white/[0.02] p-6 rounded-2xl border border-white/[0.05]">
                     <div className="flex justify-between items-center mb-6">
                       <h3 className="text-base font-black text-white uppercase italic tracking-tight">{language === 'ar' ? "وسائل الدفع" : "Payment Methods"}</h3>
                     </div>

                     <div className="space-y-4">
                       <div className="flex items-center justify-between p-4 bg-white/[0.03] rounded-xl border border-white/[0.05]">
                         <div>
                           <p className="text-white font-black text-sm uppercase">Stripe</p>
                           <p className="text-slate-500 text-[9px] font-medium mt-0.5">Credit cards, Apple Pay, Google Pay</p>
                         </div>
                         <label className="relative inline-flex items-center cursor-pointer">
                           <input type="checkbox" className="sr-only peer" checked={!!settings.businessSettings?.paymentKeys?.stripe} onChange={e => {
                             const current = settings.businessSettings?.paymentKeys || {};
                             updateSettings({...settings, businessSettings: {...settings.businessSettings, paymentKeys: {...current, stripe: e.target.checked ? {publishableKey: '', secretKey: '', webhookSecret: ''} : undefined}}});
                           }} />
                       ...
                       {settings.businessSettings?.paymentKeys?.stripe && (
                         <div className="p-4 bg-black/40 rounded-xl border border-white/[0.05] space-y-3">
                           <input value={settings.businessSettings.paymentKeys.stripe.publishableKey} onChange={e => updateSettings({...settings, businessSettings: {...settings.businessSettings, paymentKeys: {...settings.businessSettings?.paymentKeys, stripe: {...settings.businessSettings!.paymentKeys!.stripe!, publishableKey: e.target.value}}}})} placeholder="pk_test_..." className="w-full bg-white/[0.03] border border-white/[0.05] rounded-xl px-4 py-2.5 text-white focus:outline-none focus:ring-1 focus:ring-cyan-500/50 transition-all text-[11px] font-mono" />
                           <input value={settings.businessSettings.paymentKeys.stripe.secretKey} onChange={e => updateSettings({...settings, businessSettings: {...settings.businessSettings, paymentKeys: {...settings.businessSettings?.paymentKeys, stripe: {...settings.businessSettings!.paymentKeys!.stripe!, secretKey: e.target.value}}}})} placeholder="sk_test_..." type="password" className="w-full bg-white/[0.03] border border-white/[0.05] rounded-xl px-4 py-2.5 text-white focus:outline-none focus:ring-1 focus:ring-cyan-500/50 transition-all text-[11px] font-mono" />
                           <input value={settings.businessSettings.paymentKeys.stripe.webhookSecret} onChange={e => updateSettings({...settings, businessSettings: {...settings.businessSettings, paymentKeys: {...settings.businessSettings?.paymentKeys, stripe: {...settings.businessSettings!.paymentKeys!.stripe!, webhookSecret: e.target.value}}}})} placeholder="whsec_..." type="password" className="w-full bg-white/[0.03] border border-white/[0.05] rounded-xl px-4 py-2.5 text-white focus:outline-none focus:ring-1 focus:ring-cyan-500/50 transition-all text-[11px] font-mono" />
                         </div>
                       )}

                       <div className="flex items-center justify-between p-4 bg-white/[0.03] rounded-xl border border-white/[0.05]">
                         <div>
                           <p className="text-white font-black text-sm uppercase">PayPal</p>
                           <p className="text-slate-500 text-[9px] font-medium mt-0.5">PayPal accounts & cards</p>
                         </div>
                         <label className="relative inline-flex items-center cursor-pointer">
                           <input type="checkbox" className="sr-only peer" checked={!!settings.businessSettings?.paymentKeys?.paypal} onChange={e => {
                             const current = settings.businessSettings?.paymentKeys || {};
                             updateSettings({...settings, businessSettings: {...settings.businessSettings, paymentKeys: {...current, paypal: e.target.checked ? {clientId: '', clientSecret: ''} : undefined}}});
                           }} />
                           <div className="w-11 h-6 bg-white/5 rounded-full peer peer-checked:bg-cyan-500 transition-all after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:after:translate-x-5"></div>
                         </label>
                       </div>
                       {settings.businessSettings?.paymentKeys?.paypal && (
                         <div className="p-4 bg-black/40 rounded-xl border border-white/[0.05] space-y-3">
                           <input value={settings.businessSettings.paymentKeys.paypal.clientId} onChange={e => updateSettings({...settings, businessSettings: {...settings.businessSettings, paymentKeys: {...settings.businessSettings?.paymentKeys, paypal: {...settings.businessSettings!.paymentKeys!.paypal!, clientId: e.target.value}}}})} placeholder="Client ID" className="w-full bg-white/[0.03] border border-white/[0.05] rounded-xl px-4 py-2.5 text-white focus:outline-none focus:ring-1 focus:ring-cyan-500/50 transition-all text-[11px] font-mono" />
                           <input value={settings.businessSettings.paymentKeys.paypal.clientSecret} onChange={e => updateSettings({...settings, businessSettings: {...settings.businessSettings, paymentKeys: {...settings.businessSettings?.paymentKeys, paypal: {...settings.businessSettings!.paymentKeys!.paypal!, clientSecret: e.target.value}}}})} placeholder="Client Secret" type="password" className="w-full bg-white/[0.03] border border-white/[0.05] rounded-xl px-4 py-2.5 text-white focus:outline-none focus:ring-1 focus:ring-cyan-500/50 transition-all text-[11px] font-mono" />
                         </div>
                       )}

                       <div className="flex items-center justify-between p-4 bg-white/[0.03] rounded-xl border border-white/[0.05]">
                         <div>
                           <p className="text-white font-black text-sm uppercase">Paymob</p>
                           <p className="text-slate-500 text-[9px] font-medium mt-0.5">Cards, Vodafone Cash, InstaPay, Fawry</p>
                         </div>
                         <label className="relative inline-flex items-center cursor-pointer">
                           <input type="checkbox" className="sr-only peer" checked={!!settings.businessSettings?.paymentKeys?.paymob} onChange={e => {
                             const current = settings.businessSettings?.paymentKeys || {};
                             updateSettings({...settings, businessSettings: {...settings.businessSettings, paymentKeys: {...current, paymob: e.target.checked ? {apiKey: '', hmacSecret: '', merchantId: '', integrationIdCard: '', integrationIdWallet: ''} : undefined}}});
                           }} />
                           <div className="w-11 h-6 bg-white/5 rounded-full peer peer-checked:bg-cyan-500 transition-all after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:after:translate-x-5"></div>
                         </label>
                       </div>
                       {settings.businessSettings?.paymentKeys?.paymob && (
                         <div className="p-4 bg-black/40 rounded-xl border border-white/[0.05] space-y-3">
                           <input value={settings.businessSettings.paymentKeys.paymob.apiKey} onChange={e => updateSettings({...settings, businessSettings: {...settings.businessSettings, paymentKeys: {...settings.businessSettings?.paymentKeys, paymob: {...settings.businessSettings!.paymentKeys!.paymob!, apiKey: e.target.value}}}})} placeholder="API Key" className="w-full bg-white/[0.03] border border-white/[0.05] rounded-xl px-4 py-2.5 text-white focus:outline-none focus:ring-1 focus:ring-cyan-500/50 transition-all text-[11px] font-mono" />
                           <input value={settings.businessSettings.paymentKeys.paymob.hmacSecret} onChange={e => updateSettings({...settings, businessSettings: {...settings.businessSettings, paymentKeys: {...settings.businessSettings?.paymentKeys, paymob: {...settings.businessSettings!.paymentKeys!.paymob!, hmacSecret: e.target.value}}}})} placeholder="HMAC Secret" type="password" className="w-full bg-white/[0.03] border border-white/[0.05] rounded-xl px-4 py-2.5 text-white focus:outline-none focus:ring-1 focus:ring-cyan-500/50 transition-all text-[11px] font-mono" />
                           <input value={settings.businessSettings.paymentKeys.paymob.merchantId} onChange={e => updateSettings({...settings, businessSettings: {...settings.businessSettings, paymentKeys: {...settings.businessSettings?.paymentKeys, paymob: {...settings.businessSettings!.paymentKeys!.paymob!, merchantId: e.target.value}}}})} placeholder="Merchant ID" className="w-full bg-white/[0.03] border border-white/[0.05] rounded-xl px-4 py-2.5 text-white focus:outline-none focus:ring-1 focus:ring-cyan-500/50 transition-all text-[11px] font-mono" />
                           <input value={settings.businessSettings.paymentKeys.paymob.integrationIdCard} onChange={e => updateSettings({...settings, businessSettings: {...settings.businessSettings, paymentKeys: {...settings.businessSettings?.paymentKeys, paymob: {...settings.businessSettings!.paymentKeys!.paymob!, integrationIdCard: e.target.value}}}})} placeholder="Integration ID (Card)" className="w-full bg-white/[0.03] border border-white/[0.05] rounded-xl px-4 py-2.5 text-white focus:outline-none focus:ring-1 focus:ring-cyan-500/50 transition-all text-[11px] font-mono" />
                           <input value={settings.businessSettings.paymentKeys.paymob.integrationIdWallet} onChange={e => updateSettings({...settings, businessSettings: {...settings.businessSettings, paymentKeys: {...settings.businessSettings?.paymentKeys, paymob: {...settings.businessSettings!.paymentKeys!.paymob!, integrationIdWallet: e.target.value}}}})} placeholder="Integration ID (Mobile Wallet)" className="w-full bg-white/[0.03] border border-white/[0.05] rounded-xl px-4 py-2.5 text-white focus:outline-none focus:ring-1 focus:ring-cyan-500/50 transition-all text-[11px] font-mono" />
                         </div>
                       )}
