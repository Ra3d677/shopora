...
const CYCLES = [
  { id: "monthly", label: "شهري", price: 9, totalPrice: 9, total: "$9", billing: "تدفع شهرياً", savings: null, popular: false },
  { id: "3-months", label: "3 شهور", price: 7, totalPrice: 21, total: "$21", billing: "تدفع كل 3 شهور", savings: "وفر $6", popular: false },
  { id: "6-months", label: "6 شهور", price: 5.5, totalPrice: 33, total: "$33", billing: "تدفع كل 6 شهور", savings: "وفر $21", popular: true },
  { id: "annual", label: "سنوي", price: 4.5, totalPrice: 54, total: "$54", billing: "تدفع سنوياً", savings: "وفر $54", popular: false },
];
        ...
        <div className="max-w-lg w-full space-y-8">
          ...
          <div className="bg-amber-500/5 border border-amber-500/20 rounded-2xl p-6 space-y-3">
            ...
            <ol className="text-slate-400 text-xs space-y-2">
              ...
              <li>2. حول <span className="text-white font-black">{cycle.total} ≈ {totalEGP(cycle.totalPrice).toLocaleString()} ج.م</span> على: <span className="text-white font-black">{VODAFONE_CASH_NUMBER}</span></li>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {CYCLES.map((c) => (
            <button key={c.id} onClick={() => setSelectedCycle(c.id)} className={`relative flex flex-col p-6 rounded-[2rem] border-2 transition-all text-left ${selectedCycle === c.id ? "border-cyan-400 bg-white/[0.04] scale-[1.02]" : "border-white/5 bg-white/[0.02] hover:border-white/20"}`}>
              ...
              <p className="text-[10px] text-yellow-400 font-black mt-1">≈ {totalEGP(c.totalPrice).toLocaleString()} ج.م</p>
